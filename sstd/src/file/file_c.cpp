...
bool sstd::file_c::freadAll(std::vector<uint8>& rawOut){
    // get hash size
    uint32 hashSize;
    if(fp.fread((uchar*)&hashSize, sizeof(char), 4)!=4){ sstd::pdbg_err("fread() was failed.\n"); return false; }
    if(hashSize>64){ return false; }
    
    // get hash value
    uint8 pHashVal[64];
    if(fp.fread((uchar*)pHashVal, sizeof(char), hashSize)!=hashSize){ sstd::pdbg_err("fread() was failed.\n"); return false; }
    ...
    if((int64)fp.fread((uchar*)&rawOut[0], sizeof(char), rawOut.size())!=rawSize){ sstd::pdbg_err("fread() was failed: %s\n", strerror(errno)); return false; }
    ...
    }else{ sstd::pdbg_err("while checking hash value, undefined hash type was selected.\n"); return false; }
    
    if(!isEqual(pHashVal, &hashNow[0], hashNow.size())){ sstd::pdbg_err("hash value is not consistent.\n"); return false; }
...
bool sstd::file_c::fwriteAll(std::vector<uint8>& rawIn){
    ...
    if(fp.fwrite((uchar*)&hashSize, sizeof(char), 4)!=4){ sstd::pdbg_err("fwrite() was failed.\n"); return false; }
    
    // write hash value
    if(fp.fwrite((uchar*)&hashVal[0], sizeof(char), hashVal.size())!=hashVal.size()){ sstd::pdbg_err("fwrite() was failed.\n"); return false; }
    
    // write raw data
    if(fp.fwrite((uchar*)&rawIn[0], sizeof(char), rawIn.size())!=rawIn.size()){ sstd::pdbg_err("fwrite() was failed.\n"); return false; }
