 ...
 */
  ...
  /*
    ...
    ----------------------------------------------------------------------------------------------
     input string         -> parse to                    -> struct typeSet
                               std::vector<std::string>   constTF   type          poi-  convert2-
                                                                    name      T   nter* builtIn~
    ----------------------------------------------------------------------------------------------
	 int                  -> "int"                       -> [F], ["int32"], ["x"], [F], [F]
	 int*                 -> "int", "*"                  -> [F], ["int32"], ["x"], [T], [F]
	 int~                 -> "int", "~"                  -> [F], ["int32"], ["x"], [F], [T]
	 int*~                -> "int", "*", "~"             -> [F], ["int32"], ["x"], [T], [T]
	 int~*                -> "int", "~", "*"             -> [F], ["int32"], ["x"], [T], [T]
    ----------------------------------------------------------------------------------------------
	       vec<T>         ->          "vec", "T"         -> [F], [ "vec" ], ["T"], [F], [F]
	       vec<T>*        ->          "vec", "T", "*"    -> [F], [ "vec" ], ["T"], [T], [F]
	 const vec<T>*        -> "const", "vec", "T", "*"    -> [T], [ "vec" ], ["T"], [T], [F]
    ----------------------------------------------------------------------------------------------
...
// needing to "#undef" these "#define"s after include inside of "sstd.hpp", in order not to pollute the namespace.

// typeNum typeName: Below lines need to be consistend with the definition of "c2py.cpp" (char num2str[][100]{...};)
#define   void_num  0
...
void split_const(struct sstd_c2py::typeSet& f, std::string& fS){
	const char* pStr=R"(const )";
	uint len=strlen(pStr);
	
	if(fS.size()<len){ f.constTF=false; return; }

	f.constTF=true;
	uint i=0;
	for(; i<len; i++){
		if(pStr[i]!=fS[i]){ f.constTF=false; return; }
	}

	// remove "const " and additional space.
	fS = sstd::removeHeadSpace((const uchar*)&fS[i]);
}
void split_pointer_convert2builtIn(struct sstd_c2py::typeSet& f, std::string& fS){
	for(int i=fS.size()-1; i>=0; i--){
		if      (fS[i]=='*'){ fS.erase(--fS.end()); f.pointer        =true;
		}else if(fS[i]=='~'){ fS.erase(--fS.end()); f.convert2builtIn=true;
...
std::vector<struct sstd_c2py::typeSet> sstd_c2py::format_str2typeSet(const std::vector<std::string>& fSList){
	...
	for(uint i=0; i<fList.size(); i++){
		...
		split_const                  (fList[i], fS);
		split_pointer_convert2builtIn(fList[i], fS); // setting "true" or "false" of pointer and convert2builtIn
		if      (strCmp(fS,  "void")){ fList[i].name=  "void"; fList[i].N_num=  void_num; if(fList[i].pointer){ sstd::pdbg_always("ERROR: \"%s\" is an unsupported type.\n", fSList[i].c_str()); return std::vector<struct sstd_c2py::typeSet>(0); }
		}else if(strCmp(fS,  "bool")){ fList[i].name=  "bool"; fList[i].N_num=  bool_num; if(fList[i].pointer&&!IsNext_Len(fSList, fList, i)){ return std::vector<struct sstd_c2py::typeSet>(0); }
		...
		}else if(strCmp(fS, "uchar")){ fList[i].name= "uchar"; fList[i].N_num= uchar_num; if(fList[i].pointer&&!IsNext_Len(fSList, fList, i)){ return std::vector<struct sstd_c2py::typeSet>(0); }
		}else if(strCmp(fS,   "int")){ fList[i].name= "int32"; fList[i].N_num= int32_num; if(fList[i].pointer&&!IsNext_Len(fSList, fList, i)){ return std::vector<struct sstd_c2py::typeSet>(0); }
		}else if(strCmp(fS,  "int8")){ fList[i].name=  "int8"; fList[i].N_num=  int8_num; if(fList[i].pointer&&!IsNext_Len(fSList, fList, i)){ return std::vector<struct sstd_c2py::typeSet>(0); }
		}else if(strCmp(fS, "int16")){ fList[i].name= "int16"; fList[i].N_num= int16_num; if(fList[i].pointer&&!IsNext_Len(fSList, fList, i)){ return std::vector<struct sstd_c2py::typeSet>(0); }
		}else if(strCmp(fS, "int32")){ fList[i].name= "int32"; fList[i].N_num= int32_num; if(fList[i].pointer&&!IsNext_Len(fSList, fList, i)){ return std::vector<struct sstd_c2py::typeSet>(0); }
		}else if(strCmp(fS, "int64")){ fList[i].name= "int64"; fList[i].N_num= int64_num; if(fList[i].pointer&&!IsNext_Len(fSList, fList, i)){ return std::vector<struct sstd_c2py::typeSet>(0); }
		}else if(strCmp(fS, "uint8")){ fList[i].name= "uint8"; fList[i].N_num= uint8_num; if(fList[i].pointer&&!IsNext_Len(fSList, fList, i)){ return std::vector<struct sstd_c2py::typeSet>(0); }
		}else if(strCmp(fS,"uint16")){ fList[i].name="uint16"; fList[i].N_num=uint16_num; if(fList[i].pointer&&!IsNext_Len(fSList, fList, i)){ return std::vector<struct sstd_c2py::typeSet>(0); }
		}else if(strCmp(fS,"uint32")){ fList[i].name="uint32"; fList[i].N_num=uint32_num; if(fList[i].pointer&&!IsNext_Len(fSList, fList, i)){ return std::vector<struct sstd_c2py::typeSet>(0); }
		}else if(strCmp(fS,"uint64")){ fList[i].name="uint64"; fList[i].N_num=uint64_num; if(fList[i].pointer&&!IsNext_Len(fSList, fList, i)){ return std::vector<struct sstd_c2py::typeSet>(0); }
		}else if(strCmp(fS, "float")){ fList[i].name= "float"; fList[i].N_num= float_num; if(fList[i].pointer&&!IsNext_Len(fSList, fList, i)){ return std::vector<struct sstd_c2py::typeSet>(0); }
		}else if(strCmp(fS,"double")){ fList[i].name="double"; fList[i].N_num=double_num; if(fList[i].pointer&&!IsNext_Len(fSList, fList, i)){ return std::vector<struct sstd_c2py::typeSet>(0); }
		}else if(strCmp(fS,   "len")){ fList[i].name=   "len"; fList[i].N_num=   len_num; if(!IsPrevious_pointer(fSList, fList, i)||IsPrevious_pChar(fSList, fList, i)){ return std::vector<struct sstd_c2py::typeSet>(0); }
			...
			if      (strCmp(type,  "vec")){ fList[i].name="vec";   fList[i].N_num=vec_num;   if(!setTandT_num(fList[i], tempT)){ sstd::pdbg_always("ERROR: \"%s\" is an unsupported type.\n", fSList[i].c_str()); return std::vector<struct sstd_c2py::typeSet>(0); }
				...
				sstd::pdbg_always("ERROR: \"%s\" is an unsupported type.\n", fSList[i].c_str()); return std::vector<struct sstd_c2py::typeSet>(0);
			}
		}else{
			sstd::pdbg_always("ERROR: \"%s\" is an unsupported type.\n", fSList[i].c_str()); return std::vector<struct sstd_c2py::typeSet>(0);
		}
...
std::string sstd_c2py::format_typeSet2str(const std::vector<struct sstd_c2py::typeSet>& fList){
	...
	//-----------------------------------------------------------------------
	// Example of "int32" format: F int32 x F F 0
	//                           /    |   |  ＼＼＼
	//                    constTF, type, <T>, *, ~, NumOfArrElements
	//-----------------------------------------------------------------------
	// Example of "int32*" format: F int32 x T F 1 L
	//                            /    |   |  ＼＼＼＼_________________.
	//                     constTF, type, <T>, *, ~, NumOfArrElements, ArrSize
	//-----------------------------------------------------------------------
	// Example of "const mat<T>*&" format: T int32 x T T 2 R C __________________________
	//                                    /    |   |  ＼＼＼＼_________________.         ＼
	//                             constTF, type, <T>, *, ~, NumOfArrElements, row length, col length
	//-----------------------------------------------------------------------
	
	std::string typeStr;
	if(fList.size()!=0){
		if(fList[0].constTF){ typeStr += "T "; }else{ typeStr += "F "; }
		typeStr += fList[0].name;
		if(fList[0].T.size()!=0){ typeStr += ' ' + fList[0].T;
		}          else         { typeStr += " x"; }
		if(fList[0].pointer        ){ typeStr += " T"; }else{ typeStr += " F"; }
		if(fList[0].convert2builtIn){ typeStr += " T"; }else{ typeStr += " F"; }
	...
	for(uint i=1; i<fList.size(); i++){
		if(fList[i].N_num==len_num){ continue; } // len element is not used on c2py.py, so this function make "argList.bin" does not have this type of element.
		
		typeStr += "\n";
		if(fList[i].constTF){ typeStr += "T "; }else{ typeStr += "F "; }
		typeStr += fList[i].name;
		if(fList[i].T.size()!=0){ typeStr += ' ' + fList[i].T;
		}          else         { typeStr += " x"; }
		if(fList[i].pointer        ){ typeStr += " T"; }else{ typeStr += " F"; }
		if(fList[i].convert2builtIn){ typeStr += " T"; }else{ typeStr += " F"; }
...
bool sstd_c2py::c2py_ret(std::vector<std::string>& inOut, sstd::file& fp, std::string& line){
	...
	uint vecLen=sstd::str2int(elements[5]);
	inOut.resize(vecLen);
	for(uint i=0; i<vecLen; i++){
		inOut[i].resize(sstd::str2int(elements[6+i]));
	}
...
#define c2py_ret_matCR(matType, type)									\
	...
	uint rows, cols;													\
	std::vector<std::string> elements = sstd::split(line.c_str(), ' ');	\
	rows=sstd::str2int(elements[6]);									\
	cols=sstd::str2int(elements[7]);									\
																		\
	/* write back */													\
...
#define builtIn_write2file(pType, Type)									\
	if(fList[i].pointer){												\
		...
		pArgList[i]=pArgBuf;											\
																		\
		if(i>=fList.size()-1){ sstd::pdbg_always("ERROR: Next of pointer type must be \"len\" of its pointer, but inputs args number is not enough.\n"); } \
...
#define vec_write2file(Type, sizeType)\
	if(fList[i].pointer){ Type* pArgBuf=va_arg(ap, Type*); fp.fwrite(&(*pArgBuf)[0], sizeof(sizeType), (*pArgBuf).size()); fList[i].arrLen.clear(); fList[i].arrLen.push_back((*pArgBuf).size()); pArgList[i]=pArgBuf;\
	}        else       { Type   argBuf=va_arg(ap, Type ); fp.fwrite(    &argBuf[0], sizeof(sizeType),     argBuf.size()); fList[i].arrLen.clear(); fList[i].arrLen.push_back(    argBuf.size()); }

#define mat_write2file(Type, sizeType)\
	if(fList[i].pointer){ Type* pArgBuf=va_arg(ap, Type*); fp.fwrite(&(*pArgBuf)[0], sizeof(sizeType), (*pArgBuf).len()); fList[i].arrLen.clear(); fList[i].arrLen.push_back((*pArgBuf).rows()); fList[i].arrLen.push_back((*pArgBuf).cols()); pArgList[i]=pArgBuf;\
	}        else       { Type   argBuf=va_arg(ap, Type ); fp.fwrite(    &argBuf[0], sizeof(sizeType),     argBuf.len()); fList[i].arrLen.clear(); fList[i].arrLen.push_back(    argBuf.rows()); fList[i].arrLen.push_back(    argBuf.cols()); }
...
std::vector<void*> sstd_c2py::getArg_and_write2file(va_list& ap, const char* writeDir_base, std::vector<std::string> fSList, std::vector<struct sstd_c2py::typeSet>& fList){
	...
	for(uint i=1; i<fList.size(); i++){ // begin 1 to avoid return value
		...
		if      (  bool_num==fList[i].N_num){ builtIn_write2file(  bool, uint32); // bool is treated as a 4 bytes type by va_arg();
			...
			if      (  bool_num==fList[i].T_num){
			...
			}else if(  char_num==fList[i].T_num){ vec_write2file(std::vector<  char>,   char);
			}else if( uchar_num==fList[i].T_num){ vec_write2file(std::vector< uchar>,  uchar);
			}else if(  int8_num==fList[i].T_num){ vec_write2file(std::vector<  int8>,   int8);
			}else if( uint8_num==fList[i].T_num){ vec_write2file(std::vector< uint8>,  uint8);
			}else if( int16_num==fList[i].T_num){ vec_write2file(std::vector< int16>,  int16);
			}else if(uint16_num==fList[i].T_num){ vec_write2file(std::vector<uint16>, uint16);
			}else if( int32_num==fList[i].T_num){ vec_write2file(std::vector< int32>,  int32);
			}else if(uint32_num==fList[i].T_num){ vec_write2file(std::vector<uint32>, uint32);
			}else if( int64_num==fList[i].T_num){ vec_write2file(std::vector< int64>,  int64);
			}else if(uint64_num==fList[i].T_num){ vec_write2file(std::vector<uint64>, uint64);
			}else if( float_num==fList[i].T_num){ vec_write2file(std::vector< float>,  float);
			}else if(double_num==fList[i].T_num){ vec_write2file(std::vector<double>, double);
			}else if(   str_num==fList[i].T_num){
				// argList.bin
				...
				sstd::pdbg_always("ERROR: \"%s\" is an unsupported type of vec<T>.\n", fSList[i].c_str()); return std::vector<void*>();
			}
		}else if(   mat_num==fList[i].N_num){
			if      (  bool_num==fList[i].T_num){ mat_write2file(sstd::mat<  bool>,   bool);
			}else if(  char_num==fList[i].T_num){ mat_write2file(sstd::mat<  char>,   char);
			}else if( uchar_num==fList[i].T_num){ mat_write2file(sstd::mat< uchar>,  uchar);
			}else if(  int8_num==fList[i].T_num){ mat_write2file(sstd::mat<  int8>,   int8);
			}else if( uint8_num==fList[i].T_num){ mat_write2file(sstd::mat< uint8>,   int8);
			}else if( int16_num==fList[i].T_num){ mat_write2file(sstd::mat< int16>,  int16);
			}else if(uint16_num==fList[i].T_num){ mat_write2file(sstd::mat<uint16>, uint16);
			}else if( int32_num==fList[i].T_num){ mat_write2file(sstd::mat< int32>,  int32);
			}else if(uint32_num==fList[i].T_num){ mat_write2file(sstd::mat<uint32>, uint32);
			}else if( int64_num==fList[i].T_num){ mat_write2file(sstd::mat< int64>,  int64);
			}else if(uint64_num==fList[i].T_num){ mat_write2file(sstd::mat<uint64>, uint64);
			}else if( float_num==fList[i].T_num){ mat_write2file(sstd::mat< float>,  float);
			}else if(double_num==fList[i].T_num){ mat_write2file(sstd::mat<double>, double);
			}else{
				sstd::pdbg_always("ERROR: \"%s\" is an unsupported type of mat<T>.\n", fSList[i].c_str()); return std::vector<void*>();
			}
		}else if( mat_r_num==fList[i].N_num){
			if      (  bool_num==fList[i].T_num){ mat_write2file(sstd::mat_r<  bool>,   bool);
			}else if(  char_num==fList[i].T_num){ mat_write2file(sstd::mat_r<  char>,   char);
			}else if( uchar_num==fList[i].T_num){ mat_write2file(sstd::mat_r< uchar>,  uchar);
			}else if(  int8_num==fList[i].T_num){ mat_write2file(sstd::mat_r<  int8>,   int8);
			}else if( uint8_num==fList[i].T_num){ mat_write2file(sstd::mat_r< uint8>,  uint8);
			}else if( int16_num==fList[i].T_num){ mat_write2file(sstd::mat_r< int16>,  int16);
			}else if(uint16_num==fList[i].T_num){ mat_write2file(sstd::mat_r<uint16>, uint16);
			}else if( int32_num==fList[i].T_num){ mat_write2file(sstd::mat_r< int32>,  int32);
			}else if(uint32_num==fList[i].T_num){ mat_write2file(sstd::mat_r<uint32>, uint32);
			}else if( int64_num==fList[i].T_num){ mat_write2file(sstd::mat_r< int64>,  int64);
			}else if(uint64_num==fList[i].T_num){ mat_write2file(sstd::mat_r<uint64>, uint64);
			}else if( float_num==fList[i].T_num){ mat_write2file(sstd::mat_r< float>,  float);
			}else if(double_num==fList[i].T_num){ mat_write2file(sstd::mat_r<double>, double);
			}else{
				sstd::pdbg_always("ERROR: \"%s\" is an unsupported type of mat_r<T>.\n", fSList[i].c_str()); return std::vector<void*>();
			}
		}else{
			sstd::pdbg_always("ERROR: \"%s\" is an unsupported type.\n", fSList[i].c_str()); return std::vector<void*>();
		}
...
#define readLenOfPointer(sizeofType)									\
	fp.fread(pArgList[i],sizeofType,fList[i].arrLen[0]);				\
	if(!checkError_WB(fp.fsize(),fList[i].arrLen[0]*sizeofType)){ return false; }
...
bool sstd_c2py::writeBack(std::vector<std::string>& lines, std::vector<void*>& pArgList, const char* writeDir_base, std::vector<std::string>& fSList, std::vector<struct sstd_c2py::typeSet>& fList){
	for(uint i=1; i<pArgList.size(); i++){ // begin 1 to avoid return value
		...
		if      (  bool_num==fList[i].N_num){ readLenOfPointer(sizeof(  bool));
		}else if(  char_num==fList[i].N_num){ readLenOfPointer(sizeof(  char)); // Python says [TypeError: 'str' object does not support item assignment] so, this might be impossible.
		}else if( uchar_num==fList[i].N_num){ readLenOfPointer(sizeof( uchar));
		}else if(  int8_num==fList[i].N_num){ readLenOfPointer(sizeof(  int8));
		}else if( uint8_num==fList[i].N_num){ readLenOfPointer(sizeof( uint8));
		}else if( int16_num==fList[i].N_num){ readLenOfPointer(sizeof( int16));
		}else if(uint16_num==fList[i].N_num){ readLenOfPointer(sizeof(uint16));
		}else if( int32_num==fList[i].N_num){ readLenOfPointer(sizeof( int32));
		}else if(uint32_num==fList[i].N_num){ readLenOfPointer(sizeof(uint32));
		}else if( int64_num==fList[i].N_num){ readLenOfPointer(sizeof( int64));
		}else if(uint64_num==fList[i].N_num){ readLenOfPointer(sizeof(uint64));
		}else if( float_num==fList[i].N_num){ readLenOfPointer(sizeof( float));
		}else if(double_num==fList[i].N_num){ readLenOfPointer(sizeof(double));
		}else if(   str_num==fList[i].N_num){
			std::string* pStr=(std::string*)pArgList[i];
			uint len=fp.fsize();
			(*pStr).resize(len);
			fp.fread(&(*pStr)[0], sizeof(char), len); // Python says [TypeError: 'str' object does not support item assignment] so, this might be impossible.
		}else if(   vec_num==fList[i].N_num){

			// bool char ucahr の扱いは要検討
