...
TEST(mv, mv_f2f){
    ...
    std::string hash_src = sstd::system_stdout("sha256sum "+tmpDir+"/01/rand.bin | cut -d \" \" -f 1"); hash_src.pop_back(); // pop_back() is removing '\n'.
    
    sstd::mv(tmpDir+"/01/rand.bin", tmpDir+"/02/rand.bin");

    std::string hash_dst = sstd::system_stdout("sha256sum "+tmpDir+"/02/rand.bin | cut -d \" \" -f 1"); hash_dst.pop_back(); // pop_back() is removing '\n'.
...
TEST(mv, mv_d2d){
    ...
    std::string hash_src = sstd::system_stdout("sha256sum "+tmpDir+"/01/a/b/rand.bin | cut -d \" \" -f 1"); hash_src.pop_back(); // pop_back() is removing '\n'.
    
    sstd::mv(tmpDir+"/01", tmpDir+"/02");

    std::string hash_dst = sstd::system_stdout("sha256sum "+tmpDir+"/02/01/a/b/rand.bin | cut -d \" \" -f 1"); hash_dst.pop_back(); // pop_back() is removing '\n'.
    ASSERT_STREQ(hash_src.c_str(), hash_dst.c_str());
    sstd::rm(tmpDir);
}
...
#define init()                                                          \
    ...
    sstd::mkdir(tmpDir+"/02");                                          \
    sstd::system("dd if=/dev/urandom of="+tmpDir+"/01/rand.bin bs=1024 count=1 > /dev/null 2>&1"); \
    std::string hash_src = sstd::system_stdout("sha256sum "+tmpDir+"/01/rand.bin | cut -d \" \" -f 1"); hash_src.pop_back();
#define fin()                                                           \
    std::string hash_dst = sstd::system_stdout("sha256sum "+tmpDir+"/02/rand.bin | cut -d \" \" -f 1"); hash_dst.pop_back(); \
    ASSERT_STREQ(hash_src.c_str(), hash_dst.c_str());                   \
    sstd::rm(tmpDir);
